...
import static awsm.domain.banking.BaseBankAccount.Status.*;
...
public abstract class BaseBankAccount<T> extends AggregateRoot<T> {
    ...
    public BaseBankAccount(AccountHolder holder, WithdrawalLimits withdrawalLimits) {
    ...
    public void lift(WithdrawalLimits newLimits) {
        checkArgument(newLimits.dailyLimit().isGreaterThanOrEqualTo(withdrawalLimits.dailyLimit()), "New daily limit cannot be less than the current limit");
        checkArgument(newLimits.monthlyLimit().isGreaterThanOrEqualTo(withdrawalLimits.monthlyLimit()), "New daily limit cannot be less than the current limit");
